...
OLED Saver — небольшое фоновое приложение для Windows 11, которое помогает продлить срок службы OLED‑мониторов, уменьшая время показа статичных элементов интерфейса и автоматически затемняя/выключая экран при бездействии.

## Как скрипт помогает OLED-монитору
...
- Есть пункт `Start with Windows`: включает/отключает автозапуск (через ярлык в папке автозагрузки).
